...
//   let apiKey = `d881e0ef30cdb089cc21654cb2bbc1a4`
...
//       <div class = pt-4 px-16 text-white text-center>Signed in as ${userName}</div>
//       <button class="pt-4 px-16 text-white text-center sign-out">Sign Out</button>
//     `
...
//       let movie = movies[i]              
//       let docRef = await db.collection('watched').doc(`${movie.id}`)
...
firebase.auth().onAuthStateChanged(async function(user) {
  if (user) {
    ...
    // Sign-out button
    document.querySelector('.sign-in-or-sign-out').innerHTML = `
      <div class = pt-4 px-16 text-white text-center>Hello ${userName}!</div>
      <button class="pt-4 px-16 text-white text-center sign-out">Sign Out</button>
    `
    document.querySelector('.sign-out').addEventListener('click', function(event) {
      console.log('sign out clicked')
